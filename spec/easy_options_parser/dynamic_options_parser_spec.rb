  ...
  context "add options" do
    ...
    context "the help text" do
      ...
      context "with a cli description set" do
        ...
        it "should print the cli description (2)" do
          easy_options_parser.cli_description = "An alternate description for my script"
          easy_options_parser.parse
    ...
    context "the parsed options" do
      ...
      context "where option name start with the same characters" do
        before do
          easy_options_parser.add_option(:my_option, :string, "My option")
          easy_options_parser.add_option(:my_second_option, :string, "Second option")
          easy_options_parser.add_option(:my_special_option, :string, "Special option")
        end
      ...
      context 'the option type' do
        context 'BigDecimal' do
          it 'should parse the value to a big decimal where set to a valid value' do
            easy_options_parser.add_option(:my_option, :big_decimal, "My option")
            easy_options_parser.set_argv([
              '-m2.45'
            ])
        ...
        context 'dir' do
          ...
          it 'should raise an error where the path represented by the value exists but is not a directory' do
            File.write(@tmp_file, ' ')
            expect{easy_options_parser.parse}.to raise_error "The path: #{@tmp_file} is not a directory"
          end
